...
pronounced, and no voice reads Chinese here (`speech.ts` says why), so the
item carries `reading` — pinyin — and startPack copies it onto the card beside
`front`. The session shows it with the answer, not above the sentence:
