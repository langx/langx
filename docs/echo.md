...
- **A cue for every pack item.** Each item carries `image: 'cue:<slug>'`, and
  ...
  sixteen phrases that are about agreeing, so 6,480 cards resolve to 372
  pictures.
  ...
  **All 372 are drawn for this app**, their shapes in `drawings.mjs` to
  ...
  **Sixteen cards carry no cue, and that is the answer rather than a gap.**
  A cue's job is to say what the sentence is about before the words do, so a
  picture that has to be decoded is worse than none — and these sixteen are
  the ones where every candidate had to be. Nine are abstractions with no
  subject to draw ("I am nothing", "All or nothing", "Nothing is holy", "Air
  is invisible", "Did you help?", "Do you have a hobby?", "I have to lose
  weight", "It's a shadow", "I'm straight"); three name a condom, which has
  no picture in this language and no emoji to name one by; and four are a
  thief, a genie, a doll and a carpet, where the drawing would either lean on
  a stereotype or say a different word than the sentence does. Adding one
  means drawing the concept, not just pointing at it: the palette is closed
  on purpose.

  The Mandarin packs came later and leave more uncued — 53 of 1,639 — for the
  same reason at a higher level: HSK 6 is where _monopoly_ and _prophecy_
  live. `content/echo/ATTRIBUTION.md` lists what they would need drawn.
...
## Chinese is named by HSK

Six Mandarin packs, `zh:hsk1` … `zh:hsk6`, one per level of the HSK 2.0
syllabus. Two things about them are unlike every other pack.

**A second name, not a second scale.** A Chinese learner measures themselves
by HSK — the exam is the syllabus most courses follow — and "Intermediate"
tells them nothing about whether a pack is HSK 3 or HSK 4. So the pack file
carries `hsk`, the list and the pack screen call it "HSK 3", and `level` is
still one of the four: HSK 2.0 was published against CEFR A1–C2, one band per
level, and `hskLanguageLevel` sends each through `CEFR_TO_LANGUAGE_LEVEL`.
HSK 3 and HSK 4 are therefore both `intermediate`, which is why the id is
`zh:hsk3` rather than `zh:intermediate` — two packs would otherwise share an
`_id` — and why `listPacks` breaks the tie on `hsk`. `echoPackFileSchema`
refuses a file whose id, level and `hsk` disagree.

**A reading on the card.** A Chinese sentence does not say how it is
pronounced, so the item carries `reading` — pinyin — and startPack copies it
onto the card beside `front`. The session shows it with the answer, not above the sentence:
reading the characters is part of what the card asks. The card screen and the
pack preview show it under the sentence. Rewriting the sentence takes it off,
for the same reason it takes the synthesised readings off.

The pinyin is drafted by `tools/echo-content/hsk/pick.py` and read before it
ships — see "A machine may read a line" in `decisions.md` for why that is not
a gloss, and `content/echo/ATTRIBUTION.md` for what the reading of these six
packs covered.

**And a voice, read from that pinyin.** Kokoro has Chinese voices and the
service never used them, because it phonemises through espeak-ng and espeak's
Mandarin comes out with its tones stripped: a speech recogniser, handed
espeak's reading of thirty-six HSK sentences, got 41% of the characters back.
misaki — Kokoro's own front end, Apache-2.0 — reads the tones, and the same
test got 97%. So Chinese is Kokoro's seventh language with its own phonemiser
(`zh_phonemes` in `apps/tts/server.py`), in the two voices that scored best,
`zf_xiaoyi` and `zm_yunxi`.

Two paths, because two kinds of card. A member's own card has no pinyin, and
the service reads it through misaki's guess — right almost always, and wrong
