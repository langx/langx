...
v1's Express + Appwrite API, replaced in v2 by `apps/api`. It stayed in scope
while it served api.langx.io; on 5 September 2026 the repo was archived on
GitHub and api.langx.io answers from v2 (its `/api/update` returns 404). The
retirement steps are in [`release-runbook.md`](release-runbook.md) →
**Retiring the v1 API**.
...
| _(private)_      | Fly.io app `langx-tts`, Flycast only, no public address       | push to `main` of this repo; `deploy-tts.yml` builds and runs `fly deploy` |
...
`langx-angular` (`functions/discord-sync/src/main.js`, the badge sync) and
...
Archived on GitHub (verified 5 September 2026): `api` (v1's API, see above),
`langx-angular` (v1 — Ionic/Angular + Capacitor + Appwrite; v0.15 on the stores
was built from it), `langx-flutter`, `db-bulk-update`,
