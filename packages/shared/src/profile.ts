...
/**
/**
 * Body of `POST /profiles/me/handle` — a username change, on the cooldown
 * `HANDLE_CHANGE_COOLDOWN_DAYS` describes.
 *
 * The *reading* schema, exactly as `onboardingProfileSchema` uses it and for
 * the same reason: `changeHandle` is the only place that can see whether the
 ...
 * `setGenderSchema` established two blocks up: "written under a condition"
 * is not something a general-purpose PATCH body can say.
 */
export const changeHandleSchema = z.object({ handle: handleSchema })
export type ChangeHandleInput = z.infer<typeof changeHandleSchema>

/**
 * Body of `PATCH /profiles/me`. Deliberately excludes `handle` (a rename is
 * on a cooldown and keeps the old name resolvable, which is why
 * `changeHandleSchema` above has a route of its own) and every
