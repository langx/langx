...
export const ru: Localized<ServerMessages> = {
  ...
  official: {
    welcome:
      'Добро пожаловать в LangX! 👋 Найдите собеседника в разделе «Поиск», напишите ему и получайте токены за практику.\n\nЧерез этот аккаунт приходят новости о LangX — заглядывайте сюда. LangX — проект с открытым кодом, и лучше он становится благодаря тому, что нам рассказывают: если что-то сломалось или есть идея, «Настройки → О приложении → Обратная связь» попадёт прямо к команде.',
    assistantOffline:
