...
export const ru: Localized<ServerMessages> = {
  push: {
    ...
    reportRewardTitle: {
      one: '{count} жетон за помощь в защите LangX 🛡️',
      few: '{count} жетона за помощь в защите LangX 🛡️',
      many: '{count} жетонов за помощь в защите LangX 🛡️',
      other: '{count} жетона за помощь в защите LangX 🛡️',
    },
    reportRewardBody: 'Мы рассмотрели твою жалобу. Спасибо.',
    ...
    lifetimeGiftTitle: '{plan} — навсегда 🎉',
    lifetimeGiftBody: {
      one: 'Это заслужил ваш {total} токен из v1.',
      few: 'Это заслужили ваши {total} токена из v1.',
      many: 'Это заслужили ваши {total} токенов из v1.',
      other: 'Это заслужили ваши {total} токена из v1.',
    },
  ...
  lifetimeGift: {
    intro: 'Поздравляем — {plan} теперь ваш навсегда.',
    earned: {
      one: 'Вы закончили v1 с {total} токеном. Именно это его и заслужило: подарок достался аккаунтам, которые построили прежний LangX. Он никогда не истекает, и платить ничего не нужно.',
      few: 'Вы закончили v1 с {total} токенами. Именно это его и заслужило: подарок достался аккаунтам, которые построили прежний LangX. Он никогда не истекает, и платить ничего не нужно.',
      many: 'Вы закончили v1 с {total} токенами. Именно это его и заслужило: подарок достался аккаунтам, которые построили прежний LangX. Он никогда не истекает, и платить ничего не нужно.',
      other:
        'Вы закончили v1 с {total} токенами. Именно это его и заслужило: подарок достался аккаунтам, которые построили прежний LangX. Он никогда не истекает, и платить ничего не нужно.',
    },
    wallet: {
      one: 'Старый баланс перенесён из расчёта сто токенов v1 к одному, так что {carried} из них уже в новом кошельке. Сегодня в нём {balance} токен.',
      few: 'Старый баланс перенесён из расчёта сто токенов v1 к одному, так что {carried} из них уже в новом кошельке. Сегодня в нём {balance} токена.',
      many: 'Старый баланс перенесён из расчёта сто токенов v1 к одному, так что {carried} из них уже в новом кошельке. Сегодня в нём {balance} токенов.',
      other:
  ...
  official: {
    ...
    welcomeBack:
      'Привет — рады, что вы вернулись. 👋 Вы были здесь ещё в первом LangX, и теперь в вашем профиле есть значок, который об этом говорит. Он есть только у тех, кто пришёл из v1, и больше ни у кого не появится.',
    welcomeBackWhatsNew:
      'Многое изменилось. Исправления стали отдельной частью приложения, за помощь другим начисляются токены, а «Обзор» ищет того, кто учит ваш язык и говорит на том, который нужен вам. Идея та же, но теперь есть место сделать её как следует — осмотритесь.',
    welcomeBackClosing:
      'Сюда приходят новости о LangX, так что заглядывать стоит. Если чего-то не хватает или что-то сломалось, «Настройки → О приложении → Обратная связь» попадёт прямо к нам. Хорошо, что вы снова с нами. 💛',
    reportReward: {
      one: 'Спасибо за жалобу. Мы её рассмотрели и добавили в кошелёк {count} жетон за то, что ты помогаешь LangX оставаться хорошим местом для учёбы. Тот, на кого подана жалоба, никогда не узнает, кто её отправил.',
      few: 'Спасибо за жалобу. Мы её рассмотрели и добавили в кошелёк {count} жетона за то, что ты помогаешь LangX оставаться хорошим местом для учёбы. Тот, на кого подана жалоба, никогда не узнает, кто её отправил.',
      many: 'Спасибо за жалобу. Мы её рассмотрели и добавили в кошелёк {count} жетонов за то, что ты помогаешь LangX оставаться хорошим местом для учёбы. Тот, на кого подана жалоба, никогда не узнает, кто её отправил.',
      other:
        'Спасибо за жалобу. Мы её рассмотрели и добавили в кошелёк {count} жетона за то, что ты помогаешь LangX оставаться хорошим местом для учёбы. Тот, на кого подана жалоба, никогда не узнает, кто её отправил.',
    },
    assistantOffline:
