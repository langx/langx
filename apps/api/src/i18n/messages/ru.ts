...
export const ru: Localized<ServerMessages> = {
  ...
  official: {
    welcome:
      'Добро пожаловать в LangX! 👋 Я — ассистент LangX. Найдите собеседника в разделе «Поиск», напишите ему и получайте токены за практику.\n\nМне можно написать в любой момент: спросить, как что-то работает, пожаловаться на кого-то или рассказать об ошибке или идее.',
    copilotSoon:
      'Copilot ещё не готов — это партнёр для практики прямо в переписке, мы его делаем. Аккаунт создан заранее, чтобы вы нашли его в первый же день.\n\nА пока напишите @langx, и я помогу.',
    assistantOffline:
      'Сейчас я не могу отвечать на сообщения. Если это терпит, попробуйте позже; если нет — напишите на {email}, там прочитает человек.',
    assistantLimit:
      'На сегодня это всё, что я могу ответить. Завтра я буду здесь, а на {email} человек ответит в любое время.',
