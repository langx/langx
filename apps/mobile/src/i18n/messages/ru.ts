...
export const ru: Localized<EnMessages> = {
  ...
  echo: {
    ...
    aboutBody:
      'Почти всё, что ты встретил сегодня, исчезнет за неделю — если не встретишь это снова. Echo возвращает фразу прямо перед тем, как она ускользнёт, и каждый раз, когда ты её вспоминаешь, следующий перерыв становится длиннее.',
    ...
    aboutLittle:
      'Язык — это тысячи слов. Echo спрашивает только те, которые ты вот-вот забудешь, поэтому десять минут в день дают больше, чем час раз в неделю.',
    aboutYours:
      'И это твои собственные фразы из твоих переписок — слова, которые ты и так хотел запомнить.',
    aboutForgot:
      'Забыл? Карточка вернётся через минуту и начнёт с коротких перерывов заново. Ничего не теряется.',
    aboutProof:
      'Интервальное повторение и самопроверка — два самых надёжных результата в исследованиях памяти, проверяемых с 1880-х годов. Здесь они оба направлены на твой язык.',
    aboutClose: 'Понятно',
  },
