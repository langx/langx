...
import { interestLabel, useDisplayNames, useT } from '../i18n'
...
export interface ProfileScreenProps {
  handle: string
  /**
   * Where the back arrow goes. Set by whoever opened this — the screen is
   * reachable from Discover, Chats, the viewer list, the leaderboard and a
   * chat header, so a single named parent would be wrong for four of the five.
   */
  from?: string
...
export function ProfileScreen({ handle, from, embedded = false, onClose }: ProfileScreenProps) {
  ...
  /*
   * Drawn in three places — the skeleton, the not-found page and the profile
   * itself — because the bar has to be there before the request lands or the
   * page jumps by its height when it does. Absent in a panel: the list it
   * would go back to is beside it, and picking somebody else replaces this.
   */
    ...
    return (
      <Screen>
        ...
        <Button
          ...
          variant="secondary"
          onPress={() => (embedded ? onClose?.() : goBackTo('/(app)/(tabs)/discover', from))}
        />
      </Screen>
    )
  }
    ...
    if (yes)
      block.mutate(user._id, {
        onSuccess: () => {
          ...
          // instead of the tab replacing itself and leaving the blocked
          // person's page in it — the same rule the thread follows.
          if (embedded) onClose?.()
